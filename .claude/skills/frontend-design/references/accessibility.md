...
- **Interactive Elements**: Every interactive element must be keyboard-accessible. Never remove the focus ring (`outline`) without providing a visible replacement using `--color-focus` (`#0066FF`).
...
- **Color Contrast**: Minimum 4.5:1 for body text, 3.0:1 for large text/UI. Body tops out at 14px on mobile and 16px on desktop, so **body never qualifies as WCAG large text — assume 4.5:1 everywhere** and only lean on the 3:1 allowance for genuine display headings. `--color-brand-ink` (`#003399`, 10.9:1) is the token for blue text on white. `--color-brand` (`#0066FF`) is 4.8:1 and legal as text, but prefer the ink token for prose. `--color-accent-ink` (`#CC6600`) is **3.8:1 — it fails the 4.5:1 body-text minimum** and is only legal at large-text/UI sizes (18px+, or bold 14px+); it must never carry body copy. `--color-accent` (`#FFCC00`) is **1.51:1 and is never a text colour on a light surface** — it is a fill that carries dark text, or a border highlight. `--color-muted` (`#475569`, 7.6:1) is the lightest legal *body-text* colour on a light surface — never mute text with opacity.
...
- [ ] Use `--color-brand-ink` (`#003399`) for blue text on light. For amber on light, use `--color-accent-ink` (`#CC6600`, 3.8:1) — large-text/UI only, never body copy — and never the raw amber fill as text.
