...
- **Interactive Elements**: Every interactive element must be keyboard-accessible. Never remove the focus ring (`outline`) without providing a visible replacement using `--color-focus` (`#FF6535`).
...
- **Color Contrast**: Minimum 4.5:1 for body text, 3.0:1 for large text/UI elements. Note: `--color-gold-ink` (`#D6450F`) is the correct token for orange text on white — not `--color-gold` (`#FF6535`), which fails AA at small sizes. The same split applies to `--color-teal-ink`, `--color-emerald-ink`, and `--color-warning-ink`.
...
```css
*:focus-visible {
  outline: 2px solid var(--color-focus); /* #FF6535 */
  outline-offset: 2px;
}
```
...
- [ ] Use `--color-gold-ink` (`#D6450F`) for orange text on white, never raw `--color-gold`.
