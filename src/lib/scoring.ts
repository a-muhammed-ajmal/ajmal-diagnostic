...
import { DimensionKey, DimensionScore, DiagnosticResult } from '@/types';
...
export function calculateResults(answers: Record<number, string>): DiagnosticResult {
  ...
  const lowestDimension = dimensions.reduce((prev, curr) =>
  ...
  return {
    ...
    totalScore,
    maxPossibleScore: 30,
    primaryConstraint: lowestDimension.key,
    primaryConstraintLabel: lowestDimension.label
  };
}
