    ...
    <Html>
      ...
      <Body style={{ backgroundColor: '#FFFFFF', fontFamily: 'Arial, sans-serif' }}>
        <Container style={{ maxWidth: '600px', margin: '0 auto', padding: '40px 20px' }}>
          <Text style={{ color: '#C6752E', fontSize: '11px', fontWeight: 'bold', letterSpacing: '3px', margin: '0 0 8px' }}>
            MUHAMMED AJMAL CONSULTING
          </Text>
          <Heading style={{ color: '#132A4A', fontSize: '24px', margin: '0 0 4px' }}>
            {firstName}, your Founder Dependency Index
          </Heading>
          <Text style={{ color: '#7C7362', fontSize: '14px', margin: '0 0 24px' }}>
            Prepared for {companyName}
          </Text>
          <Section style={{ backgroundColor: '#132A4A', borderRadius: '12px', padding: '24px', marginBottom: '16px' }}>
            <Text style={{ color: '#C6752E', fontSize: '10px', fontWeight: 'bold', letterSpacing: '2px', margin: '0 0 8px' }}>
            ...
            <Text style={{ color: '#E0964F', fontSize: '16px', fontWeight: 'bold', margin: '8px 0 0' }}>
              {report.index.band.label}
            </Text>
          </Section>
          <Heading style={{ color: '#132A4A', fontSize: '18px', margin: '24px 0 12px' }}>
            Component scores
          </Heading>
          {report.components.map((component) => (
            <Text key={component.key} style={{ color: '#132A4A', fontSize: '14px', lineHeight: '1.6', margin: '4px 0' }}>
              <strong>{component.label}:</strong> {component.display} / 100
            </Text>
          ))}
          <Text style={{ color: '#132A4A', fontSize: '14px', lineHeight: '1.6', margin: '16px 0 0' }}>
            Dependency appears most concentrated in {concentration}.
          </Text>
          {alerted.length > 0 && (
            <Text style={{ color: '#8C4D1F', fontSize: '14px', lineHeight: '1.6', margin: '8px 0 0' }}>
          ...
          {report.observations.map((finding) => (
            <Text key={finding} style={{ color: '#132A4A', fontSize: '14px', lineHeight: '1.6', margin: '8px 0' }}>
              • {finding}
            </Text>
          ))}
          <Text style={{ color: '#7C7362', fontSize: '13px', fontStyle: 'italic', lineHeight: '1.6', margin: '24px 0' }}>
            {report.limitation}
          </Text>
          <Button href={calendlyLink} style={{ backgroundColor: '#C6752E', borderRadius: '8px', color: '#132A4A', fontSize: '14px', fontWeight: 'bold', padding: '12px 18px', textDecoration: 'none' }}>
            Discuss an Audit
          </Button>
        </Container>
      </Body>
