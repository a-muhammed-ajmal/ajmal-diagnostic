import {
  Html, Head, Body, Container, Heading, Text, Section, Hr, Button, Preview
...
export function DiagnosticReportEmail({ name, companyName, results, calendlyLink }: DiagnosticReportEmailProps) {
  ...
  const severityColors: Record<string, string> = {
    Critical: '#E11D48', Developing: '#D97706', Progressing: '#10B981'
  };
    ...
    <Html>
      ...
      <Body style={{ backgroundColor: '#FFFFFF', fontFamily: 'Arial, sans-serif' }}>
        <Container style={{ maxWidth: '600px', margin: '0 auto', padding: '40px 20px' }}>
          ...
          <Section style={{ backgroundColor: '#1e293b', borderRadius: '12px', padding: '20px', marginBottom: '24px' }}>
            <Text style={{ color: '#94a3b8', fontSize: '10px', fontWeight: 'bold', letterSpacing: '2px', margin: '0 0 8px' }}>SECONDARY CONSTRAINT</Text>
            <Text style={{ color: '#ffffff', fontSize: '16px', fontWeight: 'bold', margin: '0 0 8px' }}>{results.secondaryConstraintLabel}</Text>
            <Text style={{ color: '#94a3b8', fontSize: '13px', lineHeight: '1.6', margin: 0 }}>{secondaryMeta.secondaryExplanation}</Text>
          </Section>
          ...
          {results.dimensions.map(dim => (
            <Section key={dim.key} style={{ marginBottom: '10px' }}>
              <Text style={{ margin: '0 0 4px', fontWeight: 'bold', color: '#374151', fontSize: '13px' }}>
                ...
                {dim.key === results.primaryConstraint && ' ← Primary Constraint'}
                {dim.key === results.secondaryConstraint && ' ← Secondary Constraint'}
              </Text>
            </Section>
          ))}
          <Hr style={{ borderColor: '#e5e7eb', margin: '24px 0' }} />
          ...
          <Section style={{ backgroundColor: '#fef3c7', border: '1px solid #fde68a', borderRadius: '12px', padding: '16px', marginBottom: '24px' }}>
            <Text style={{ color: '#92400e', fontSize: '13px', lineHeight: '1.6', margin: 0 }}>
              <strong>An important distinction:</strong> This diagnostic identifies your constraint category — not the root cause within it. Understanding exactly why this constraint exists in your specific business is what the paid diagnostic is designed to uncover.
            </Text>
          </Section>
          ...
          <Text style={{ color: '#9ca3af', fontSize: '11px', textAlign: 'center' }}>Muhammed Ajmal Consulting · Dubai, UAE · Strategic Growth Architecture™</Text>
        </Container>
      </Body>
    </Html>
  );
