...
export default async function AdminFdiDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="min-h-screen bg-brand-tint"><nav className="border-b border-line bg-white text-ink px-6 py-4 flex items-center justify-between"><div className="flex items-center gap-4"><Link href="/admin/fdi" className="text-xs text-muted hover:text-brand-ink">← FDI sessions</Link><span className="font-heading font-bold text-[length:var(--step-0)]">M<span className="text-brand-ink">A</span> · FDI record</span></div><a href="/api/admin/logout" className="text-xs text-muted hover:text-brand-ink">Sign Out</a></nav><section className="max-w-5xl mx-auto px-4 py-8 space-y-5"><header className="bg-white border border-line rounded-2xl p-6"><div className="flex flex-col md:flex-row md:justify-between gap-4"><div><p className="eyebrow text-brand-ink">{session.is_test ? 'Explicit test record' : 'Live FDI record'}</p><h1 className="font-heading font-extrabold text-ink text-[length:var(--step-3)] mt-2">{session.name ?? 'Incomplete session'}</h1><p className="font-body text-muted mt-1">{session.company_name ?? 'No company captured'} · {session.email ?? 'No email captured'}</p></div><div className="md:text-right"><p className="font-heading font-extrabold text-ink text-[length:var(--step-4)]">{session.fdi_display === null ? '—' : `${session.fdi_display} / 100`}</p><p className="font-body text-[length:var(--step-0)] text-muted">{session.band_label ?? session.status}</p></div></div><div className="grid grid-cols-2 md:grid-cols-4 gap-4 border-t border-line pt-5 mt-5"><div><p className="eyebrow text-muted">Decision Speed</p><p className="font-heading font-bold text-ink text-[length:var(--step-0)] mt-1">{session.display_ds ?? '—'}</p></div><div><p className="eyebrow text-muted">Execution</p><p className="font-heading font-bold text-ink text-[length:var(--step-0)] mt-1">{session.display_ec ?? '—'}</p></div><div><p className="eyebrow text-muted">Visibility</p><p className="font-heading font-bold text-ink text-[length:var(--step-0)] mt-1">{session.display_ov ?? '—'}</p></div><div><p className="eyebrow text-muted">Email</p><p className="font-heading font-bold text-ink text-[length:var(--step-0)] mt-1">{session.status === 'completed' ? (session.email_sent ? 'Sent' : 'Pending') : '—'}</p></div></div></header>
      ...
      <section className="bg-white border border-line rounded-2xl p-6"><h2 className="font-heading font-extrabold text-ink text-[length:var(--step-0)]">Saved FDI answers</h2><div className="overflow-x-auto mt-4"><table className="w-full text-[length:var(--step-0)]"><thead><tr className="border-b border-line"><th className="text-left py-2 font-heading text-xs text-muted">Question</th><th className="text-left py-2 font-heading text-xs text-muted">Component</th><th className="text-left py-2 font-heading text-xs text-muted">Score</th><th className="text-left py-2 font-heading text-xs text-muted">Changes</th></tr></thead><tbody>{((answers ?? []) as FdiAnswer[]).map((answer) => <tr key={answer.question_id} className="border-b border-line"><td className="py-2 font-body text-ink">{answer.question_id}</td><td className="py-2 font-body text-muted">{answer.component_key}</td><td className="py-2 font-heading font-bold text-ink">{answer.score}</td><td className="py-2 font-body text-muted">{answer.change_count}</td></tr>)}</tbody></table></div></section>
    </section></div>
  );
}
